...
def draw_pmi(ax, fig, pmi, ac_labels, sem_labels):
    ...
    infant_c  = [i for i, s in enumerate(sem_c) if s in {"distress","submission","begging"}]
    contact_c = [i for i, s in enumerate(sem_c) if s in {"contact","group_coordination","affiliation","attention","spacing"}]
