...
Current status: the subset contains 58 species. We have retrieved 39 inventory-paper PDFs so far: 13 automatically with Codex and 26 manually from the HTML table. The remaining 19 selected inventory papers still need verified PDFs.
...
Audit update, 2026-05-21: the selected subset was rechecked against currently excluded species with candidate inventory papers. Three previously excluded species now pass the threshold and were promoted: `Indri indri` (Maretti et al. 2010 explicitly treats the song plus eight additional adult vocal types), `Saccopteryx bilineata` (Behr and von Helversen 2004 classifies seven wild adult male day-roost vocalization types), and `Urocitellus beldingi` (Leger et al. 1984 treats wild vocalizations across predator, post-copulatory, and agonistic contexts). Candidate spines for the remaining excluded species were left excluded when they were alarm-only, song-only, food-call-only, courtship-only, captive/secondary-only, not species-specific enough, or too narrow relative to the full-inventory threshold. PDFs for all three promoted species have been manually retrieved and verified; the `Urocitellus beldingi` PDF is a ScienceDirect legacy scan/OCR copy.
