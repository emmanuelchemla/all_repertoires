...
- LLM agents grow the database using `.agents/skills/species-repertoire/SKILL.md`, which defines the curation rules.
- The schema and validator live with that skill. Before committing, an agent always validates with `python .agents/skills/species-repertoire/validate.py repertoires/species/<file>.yaml` from the repository root.
...
```
Follow `.agents/skills/species-repertoire/SKILL.md` to add 10 new species.
```
